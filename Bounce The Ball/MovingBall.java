...
public class MovingBall {
    ...
    public int x_velocity = +3; // horizonal speed; positive is to the right
    public int y_velocity = +2; // vertical speed; positive is downwards
    ...
    public void move(int time_units) {
        ...
        if (container.inHorizontalContact(x_pos) && (container.inHorizontalContactx(x_pos + radiusOf()))) {
            x_velocity = -x_velocity;
        }   // reverse horizontal direction
        y_pos = y_pos + (y_velocity * time_units);
        if (container.inVerticalContact(y_pos) && (container.inVerticalContacty(y_pos + radiusOf()))) {
